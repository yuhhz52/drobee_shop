...
const dataProvider = withLifecycleCallbacks(customProvider, [
  {
    ...
    beforeSave: async (params) => {
      ...
      return {
        ...requestBody,
        productResources: normalizeProductResources(productResList, requestBody.name),
      };
    },
  },
]);

function normalizeProductResources(resources = [], productName) {
  const normalized = (resources || []).map((resource) => ({
    ...resource,
    type: resource?.type ? String(resource.type).toUpperCase() : 'IMAGE',
  }));

  const hasPrimary = normalized.some((resource) => resource?.isPrimary);
  if (!hasPrimary && normalized.length > 0) {
    normalized[0] = {
      ...normalized[0],
      name: normalized[0].name || (productName ? `${productName} Primary` : 'Primary'),
      isPrimary: true,
    };
  }
